...
def find_max_cpu_load(jobs):
  ...
  total = jobs[0].cpu_load
  job = jobs[0]
  for i in range(1, len(jobs)):
    if overlap(job, jobs[i]) :
      job = merge(job, jobs[i])
    else :
      job = jobs[i]
    total = max(total, job.cpu_load)
  return total

def overlap (j1,j2) :
  if j1.start > j2.end or j2.start > j1.end :
    return False
  return True

def merge(j1,j2) :
  return job(min(j1.start, j2.start), max(j1.end, j2.end), j1.cpu_load + j2.cpu_load)
