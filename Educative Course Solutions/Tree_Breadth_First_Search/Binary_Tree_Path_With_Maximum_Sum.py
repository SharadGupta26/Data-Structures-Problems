...
def find_maximum_path_sum(root):
  ...
  if root.left is None and root.right is None :
    return root.val
  val = treemax(root.left) + treemax(root.right) + root.val
  return max(val, find_maximum_path_sum(root.left),find_maximum_path_sum(root.right))
...
def treemax(root) :
  ...
  if root.left is None and root.right is None :
    return root.val
  return max(treemax(root.left), treemax(root.right)) + root.val
